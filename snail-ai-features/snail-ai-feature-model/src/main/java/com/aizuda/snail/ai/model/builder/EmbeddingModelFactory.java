...
/**
 * SnailEmbeddingModel 工厂接口 - 支持不同提供商的 SnailEmbeddingModel 创建
...
public interface EmbeddingModelFactory {
    ...
    /**
     * 根据提供商和配置创建 SnailEmbeddingModel
     *
     * @param providerKey   提供商标识 (如 "openai")
     * @param baseUrl       API基础URL (如 https://api.openai.com/v1)
     * @param apiKey        解密后的API Key
     * @param modelKey      模型标识符 (如 "text-embedding-3-large")
     * @param configJson    配置JSON (包含维度等参数)
     * @return 创建好的 SnailEmbeddingModel
