...
| 嵌入式聊天 Token | 已支持 | `snail-ai-server/snail-ai-server-openapi/src/main/java/com/aizuda/snail/ai/openapi/controller/OpenApiEmbedController.java` |
| 独立 RAG 搜索 OpenAPI | 当前未提供独立路径 | 通过绑定 RAG 的智能体对话使用知识库能力 |
...
应用 ID 和 Token 在管理后台的应用管理中创建和复制。`Snail-Ai-Auth` 是 Admin API 与嵌入式聊天会话使用的认证头，不是 OpenAPI 外部集成的通用认证头。
...
| 嵌入 Token | `GET/POST /embed-token` | 生成嵌入式聊天 Token |
