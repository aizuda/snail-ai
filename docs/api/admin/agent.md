...
智能体（Agent）是 Snail AI 的核心资源，提供创建、配置、对话、数据分析、市场订阅及可观测性追踪等完整能力。
...
| `bookmarked`     | `boolean` | 否   | 是否只查收藏的         |
| `status`         | `number`  | 否   | 状态过滤               |
| `conversationId` | `string`  | 否   | 按对话 ID 过滤         |

**curl 示例：**

```bash
curl -X GET 'http://localhost:8900/snail-ai/agent/1/traces?page=1&size=10&bookmarked=true' \
  -H 'Snail-Ai-Auth: eyJhbGciOiJIUzI1NiJ9...'
```
...
```
GET /snail-ai/agent/{id}/trace/{traceId}
```

**curl 示例：**

```bash
curl -X GET 'http://localhost:8900/snail-ai/agent/1/trace/trace-uuid-001' \
  -H 'Snail-Ai-Auth: eyJhbGciOiJIUzI1NiJ9...'
```
...
```json
{
  ...
  "data": {
    "id": "trace-uuid-001",
    "input": {"content": "帮我查询退款政策"},
    "output": {"content": "根据公司退款政策..."},
    "model": "gpt-4o",
    "startTime": 1717200000000,
    "endTime": 1717200002500,
    "durationMs": 2500,
    "status": 1,
    "bookmarked": true,
    "observations": [
      {
        ...
        "model": "gpt-4o",
        "startTime": 1717200000100,
        "durationMs": 1800,
        "usageDetails": {"input_tokens": 1200, "output_tokens": 800},
        "totalCost": 0.05,
        "toolCalls": [
          {"id": "tc-001", "type": "function", "name": "rag_search", "arguments": "{\"query\":\"退款政策\"}"}
        ]
      },
      {
        "id": "obs-002",
...
```
GET /snail-ai/agent/{agentId}/conversation/{conversationId}/trace
```

**curl 示例：**

```bash
curl -X GET 'http://localhost:8900/snail-ai/agent/1/conversation/conv-abc123/trace' \
  -H 'Snail-Ai-Auth: eyJhbGciOiJIUzI1NiJ9...'
```
...
```
PUT /snail-ai/agent/trace/{traceId}/bookmark
```

**curl 示例：**

```bash
curl -X PUT 'http://localhost:8900/snail-ai/agent/trace/trace-uuid-001/bookmark' \
  -H 'Snail-Ai-Auth: eyJhbGciOiJIUzI1NiJ9...'
```
...
为 Trace 或 Observation 创建评分。

```
POST /snail-ai/agent/score
```
...
| `name`          | `string` | 是   | 评分名称（如 `accuracy`、`helpfulness`）      |
| `value`         | `number` | 否   | 数值评分                                      |
| `stringValue`   | `string` | 否   | 文本/分类评分                                 |
| `dataType`      | `string` | 是   | `NUMERIC` / `CATEGORICAL` / `BOOLEAN` / `TEXT`|
| `source`        | `string` | 否   | `ANNOTATION` / `API` / `EVAL`                 |
| `comment`       | `string` | 否   | 评分备注                                      |

**curl 示例：**

```bash
curl -X POST 'http://localhost:8900/snail-ai/agent/score' \
  -H 'Content-Type: application/json' \
  -H 'Snail-Ai-Auth: eyJhbGciOiJIUzI1NiJ9...' \
  -d '{
...
```
DELETE /snail-ai/agent/score/{scoreId}
```

**curl 示例：**

```bash
curl -X DELETE 'http://localhost:8900/snail-ai/agent/score/score-uuid-001' \
  -H 'Snail-Ai-Auth: eyJhbGciOiJIUzI1NiJ9...'
```
