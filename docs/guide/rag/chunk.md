...
<!-- screenshot: rag-chunk-list.png — 分片列表页面，左侧展示卡片视图（两列网格，每张卡片显示序号、内容预览、来源文档、字符数），右侧展示列表视图切换按钮 -->

## 分片策略

Snail AI 提供 4 种分片策略，适用于不同类型的文档和场景：

<!-- screenshot: rag-chunk-config.png — 知识库创建/编辑抽屉中的分片策略配置区域，展示四种策略的选择卡片（默认递归、分隔符、正则、智能）和对应的参数面板 -->
